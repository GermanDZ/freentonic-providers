...
# Golden-parity gate for the Revolut normalizer migration (Ruby →
# normalize: plan:). For every raw payload in test/fixtures/*.json, the
# plan the workflow now declares must reproduce the canonical payload the
# old normalizer.rb produced — captured in test/golden/*.json via
# `rake golden:dump[revolut]` while the Ruby normalizer was still live.
#
...
# stripped on both sides so the comparison is byte-for-byte deterministic.
class RevolutNormalizeParityTest < Minitest::Test
  ...
  # One assertion per fixture, named so a failure points at the fixture.
  ...
  def test_the_workflow_actually_declares_a_plan
    # Guard: if the workflow silently reverted to ruby:, the parity above
    # would pass trivially against the same normalizer that made the
    # goldens. Assert we're exercising the declarative path.
